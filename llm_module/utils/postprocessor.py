...
def postprocess_goal(goal, cfg):
    if cfg.deterministic:
        pass
    else:
        new_goal = np.zeros([goal.shape[0], cfg.best_of_n, 1, 2])
        for ped_id in tqdm(range(goal.shape[0]), desc="Postprocess"):
            # goal_filtered = goal[ped_id, ~mask]
            goal_filtered = goal[ped_id].copy()
            max_samples_filtered = goal_filtered.shape[0]

            # Clustering
            if max_samples_filtered > cfg.best_of_n:
                temp = goal_filtered.reshape(max_samples_filtered, -1)
                centroids = KMeans(n_clusters=cfg.best_of_n, random_state=0, init='k-means++', n_init=1).fit(
                    temp).cluster_centers_
                goal_filtered = centroids.reshape(cfg.best_of_n, 1, -1)

            new_goal[ped_id, :goal_filtered.shape[0]] = goal_filtered
