...
def text2goal(description: str, dim=2):    
    # Sanity check
    error = False
    if len(description) == 0:
        # Empty description
    ...
    if not error:
        try:
            description_cleanup = description[description.find('('):description.find(')') + 1]
            description_list = eval(description_cleanup)
    ...
    # validity check
    if not error and len(description_list) == 2:
        try:
            traj = np.expand_dims(np.array(description_list), axis=0).astype(np.float32)
        except:
            error = True
    else:
        error = True
...
def batch_text2goal(desc_list):
    return [text2goal(desc) for desc in desc_list]
