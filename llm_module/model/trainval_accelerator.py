...
def trainval(cfg):
    ...
    if cfg.train_dataset_type is not None:
        preprocessed_train_dataset_name = f"{cfg.dataset_name}-train-{cfg.obs_len}-{cfg.pred_len}-{cfg.metric}-{cfg.train_dataset_type}.json"
    else:
    ...
    if cfg.valid_dataset_type is not None:
        preprocessed_val_dataset_name = f"{cfg.dataset_name}-val-{cfg.obs_len}-{cfg.pred_len}-{cfg.metric}-{cfg.valid_dataset_type}.json"
    else: 
    ...
    if cfg.resume_from_checkpoint:
        ...
        training_difference = cfg.resume_from_checkpoint
    ...
    for epoch in range(starting_epoch, cfg.num_train_epochs):
        ...
        if cfg.checkpointing_steps == "epoch" and epoch % cfg.save_every == 0:
