...
# from src.models.model_utils.cnn_big_images_utils import make_gaussian_map_patches
...
def TTST_test_time_sampling_trick(x, num_goals, device, static=None, start_position=None, last_position=None):
    """
    ...
    num_mid_positions = 2
    if num_clusters > num_mid_positions:
        num_clusters -= num_mid_positions
    else:
        num_mid_positions = 0
    for person in range(goal_samples.shape[1]):
        ...
        if static[person]:
            start, end = start_position[person].unsqueeze(0), last_position[person].unsqueeze(0)
        else:
            start, end = goal_samples_argmax[person], last_position[person].unsqueeze(0)
            
        if num_mid_positions > 0:
            t = torch.linspace(0, 1, num_mid_positions+1, device=device).view(-1, 1)
            mid_positions = (1 - t) * start + t * end
            mid_positions = mid_positions[1:]
            cluster_centers = torch.cat([cluster_centers, mid_positions], dim=0)
