...
class trainer(object):
    ...
    def _initialize_network(self, model_name: str):
        ...
        """
        if model_name == 'VisSem':
            from src.models.VisSem import VisSem
            net = VisSem(self.args, self.device)
        else:
    ...
    def _set_scheduler(self, scheduler_name: str):
        ...
        # Exponential
            ...
            print(f"ExponentialLR scheduler with gamma={self.args.gamma}")
    ...
    def _load_or_restart(self):
        ...
        if self.args.load_checkpoint is not None and self.args.model_name != 'Dir':
    ...
    def _train_loop(self, start_epoch, end_epoch):
        ...
        if wandb.run is None and self.args.use_wandb:
            wandb.init(settings=wandb.Settings(start_method="thread"),
                       project="GoalSAR", config=self.args, entity="sskim0126",
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            if self.args.save_every == 0 or epoch % self.args.save_every == 0:
            ...
            if epoch >= self.args.start_validation and \
                ...
                for k, v in self.best_metrics.items():
                    ...
                    if k == 'Dir_Acc':
                        current_metric_loss = 1 - current_metric_loss
                        v = 1 - v
            ...
            if self.scheduler is not None:
                if self.args.scheduler == 'ReduceLROnPlateau':
                    if epoch >= self.args.start_validation and \
                            epoch % self.args.validate_every == 0:
                        lr_sched_metric = valid_metrics["valid_FDE"]
    ...
    def _train_epoch(self, epoch):
        ...
        for inputs, ground_truth, batch_id in train_bar:
            inputs = {k: v.squeeze(0).to(self.device) for k, v in inputs.items()}
            ground_truth = ground_truth[0].to(self.device)

            seq_list = inputs["seq_list"]
            scene_name = batch_id["scene_name"][0]
            inputs["scene"] = self.net.dataset.scenes[scene_name]
    ...
    def _evaluate_epoch(self, epoch, mode='valid'):
        ...
        if mode == 'test' and self.args.model_name == 'CLIPGoal':
            save_goal = True
            goals = []
        else:
            save_goal = False
        if mode == 'test' and self.args.model_name == 'Dir':
            save_dir = True
            dirs = []
        else:
            save_dir = False
        # loop over batches
        for idx, (inputs, ground_truth, batch_id) in enumerate(evaluate_bar):
            if idx % self.args.eval_every != 0:
                continue
            inputs = {k: v.squeeze(0).to(self.device) for k, v in inputs.items()}
            ground_truth = ground_truth[0].to(self.device)
            ...
            if save_goal:
                goals.append(all_aux_outputs["goal_point"])
            if save_dir:
                dirs.append(F.softmax(all_aux_outputs["pred_directions"][0], dim=1))
        ...
        if save_goal:
            ...
            np.save(os.path.join(self.args.model_dir, f'{self.args.test_set}-test-goals-{self.args.num_test_samples}.npy'), goals.cpu().detach().numpy())

        if save_dir:
            dirs = torch.cat(dirs, dim=0)
            np.save(os.path.join(self.args.model_dir, f'{self.args.test_set}-test-dirs.npy'), dirs.cpu().detach().numpy())
